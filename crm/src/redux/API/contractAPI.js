...
export async function getSubContractListAPI(page, pageNumber, status, search) {
  try {
    let result;
    if (search && status === undefined) {
      let newSearchData = { ...search };
      ...
      result = await AxiosExpress({
        url: `${local}/api/contract/subcontractlist?page_size=${pageNumber}&page=${page}&sort_by=id&order=desc&search=true${queryString}`,
        method: "GET",
        data: newSearchData,
      });
    } else if (status !== undefined) {
      ...
      result = await AxiosExpress({
        url: `${local}/api/contract/subcontractlist?page_size=${pageNumber}&page=${page}&status=${status}&sort_by=id&order=desc${queryString}`,
        method: "GET",
      });
    } else {
      result = await AxiosExpress({
        url: `${local}/api/contract/subcontractlist?page_size=${pageNumber}&page=${page}&sort_by=id&order=desc`,
        method: "GET",
      });
    }
    return result.data;
...
export async function createContractAPI(data) {
  try {
    let convertBeginDate = moment(data.contract.begin_date).format(
      "YYYY-MM-DD"
    );
...
export async function updateRequestAPI(data) {
  try {
    let updateRequest = {
      quality: data.quality,
      custom_price:
        data.custom_price >= 1000000 ? data.custom_price / 1000000 : null,
    };
    const result = await AxiosExpress({
...
export async function createDetailAPI(data) {
  try {
    let newDetail = {
      contract_ID: +data.contract_id,
      sub_contract_ID:
        data.sub_contract_id && data.sub_contract_id?.length > 0
...
export async function getSubContractRequestAPI(
  contract_ID,
  sub_contract_id,
  request_done
) {
