  ...
  const calculateGiaSauChietKhau = (row) => {
    if (row === 1) {
      if (valueForm.discount_by_percent === 0) {
        ...
        }).format(valueForm.total);
      } else {
        let giaSauChietKhau =
          valueForm.total -
          (valueForm.total * valueForm.discount_by_percent) / 100;
        return new Intl.NumberFormat("vi-VN", {
          maximumFractionDigits: 0,
        }).format(giaSauChietKhau);
      }
    } else if (row === 2) {
      if (calculateGTTH("number") === "") {
        ...
        let giaSauChietKhau =
          giaTriThucHien -
          (giaTriThucHien * valueForm.discount_by_percent) / 100;
        return new Intl.NumberFormat("vi-VN", {
          maximumFractionDigits: 0,
        }).format(giaSauChietKhau);
      }
    ...
    if (row === 1) {
      if (valueForm.total_include_VAT >= 0 && !isEdit) {
        ...
        return new Intl.NumberFormat("vi-VN").format(
          valueForm.total_include_VAT * 1000000
        );
      } else {
        ...
        if (giaTriSauThue > 0 && +VAT === 10) {
          ...
          return new Intl.NumberFormat("vi-VN", {
            maximumFractionDigits: 0,
          }).format(doanhThu.toFixed(6) * 1000000);
        }
      }
    } else if (row === 2) {
      let giaTriSauThue = valueForm.total / 1000000;
      ...
      if (discount !== 0) {
        if (giaTriSauThue > 0 && +VAT === 10) {
          ...
          return new Intl.NumberFormat("vi-VN").format(
            doanhThuRow2.toFixed(6) * 1000000
          );
        } else {
          let giaTruocThue = giaTriSauThue / 1.1;
          ...
          return new Intl.NumberFormat("vi-VN", {
            maximumFractionDigits: 0,
          }).format(doanhThuRow2.toFixed(6) * 1000000);
        }
      }
      return "";
      ...
      <div className="field__input_3">
        <div className="contract__field">
          ...
          >
            <input
              style={{
                backgroundColor: unlockInput ? "#fafafa" : "transparent",
                cursor: unlockInput ? "default" : "",
              }}
              className="style"
              type="text"
              name="total"
