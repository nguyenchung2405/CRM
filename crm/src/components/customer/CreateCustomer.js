...
export default function CreateCustomer({ onCancelMoal }) {
  ...
  let [validateForm, setValidateForm] = useState();
  const { isCreateCustomer, dataCustomer, customerTypeList, jobTypeList } =
  ...
  const handleCancel = () => {
    ...
    setValueForm({});
    !onCancelMoal ? history.replace(`${uri}/crm/customer`) : onCancelMoal();
  };
  ...
  const handleOk = async () => {
    ...
    if (!check) {
      if (
        window.location.href.includes("/customer/create") ||
        window.location.href.includes("/contract/create")
      ) {
        setTimeout(() => {
          if (onCancelMoal) {
            dispatch({
              type: CREATE_CUSTOMER_MODAL,
              data: valueForm,
            });
            onCancelMoal();
          } else {
  ...
  const checkValueForm = () => {
    ...
    let newValiDateForm = valueForm.is_company ? { ...validateForm } : {};
    for (let vali in newValiDateForm) {
      if (vali.includes("phone")) {
        if (
          valueForm[vali] &&
          valueForm[vali] !== "" &&
          valueForm[vali] !== undefined
        ) {
    ...
    if (validateForm[nameOfField] && nameOfField.includes("phone")) {
      return (
        <p className="required__field">
          * Số điện thoại không hợp lệ. (Bắt đầu bằng 0 hoặc 84 + 9 số)
        </p>
      );
  ...
  const renderInforBusiness = () => {
    return (
      <div className={!valueRadio ? "showInforBusiness" : ""}>
